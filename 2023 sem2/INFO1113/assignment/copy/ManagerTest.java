...
public class ManagerTest {
    ...
    public void test_loop_Normal1(){
        ...
        assertEquals(2,test_manager.Monsters.size());
    ...
    public void testDisplayWaveInfoNormal() {
        ...
        assertTrue(result.startsWith("Wave 1 in:")); // 确保在Normal模式下，我们是从第一波开始的。

        test_manager.set_current_wave(3); // 假设总共有5波
        result = test_manager.displayWaveInfo(0, 0);
        assertTrue(result.startsWith("Wave 4 in:")); // 确保在Normal模式下，波次数是正确的。
    }

    @Test
    public void testDisplayWaveInfoInfinite() {
        Inf_test_manager.setMode("Infinite");
        Inf_test_manager.set_current_wave(0);

        String result = Inf_test_manager.displayWaveInfo(0, 0);
        assertTrue(result.startsWith("Wave 2 in:")); // 确保在Infinite模式下，我们是从第二波开始的。

        Inf_test_manager.set_current_wave(3);
        result = Inf_test_manager.displayWaveInfo(0, 0);
        assertTrue(result.startsWith("Wave 5 in:")); // 确保在Infinite模式下，波次数是正确的。
    }
    ...
    public void testLoopInf(){
        ...
        assertEquals(1,Inf_test_manager.Monsters.size());
